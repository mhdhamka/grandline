...
export const MangaBubble: React.FC<MangaBubbleProps> = ({
  ...
  const renderSvgBorderAccents = () => {
    if (isTerminal) {
      // Vegapunk Satellite Interface SVG Decals
      return (
        <svg
    ...
    if (effectiveShape === 'shout') {
      // Manga Action Spikes Corner Accents
      return (
        <div className="absolute inset-0 pointer-events-none -m-1.5 overflow-visible z-0">
          <svg className="w-full h-full overflow-visible" preserveAspectRatio="none">
            {/* Action burst notches */}
            <polygon
              points="-4,15 0,8 -6,0 8,0 15,-4"
              fill="#000000"
            />
            <polygon
              points="calc(100% - 15px),-4 calc(100% - 8px),0 calc(100% + 6px),0 100%,8 calc(100% + 4px),15"
              fill="#000000"
            />
          </svg>
        </div>
  ...
  const renderThoughtTrail = () => {
    ...
    return (
      ...
      >
        <div
          className={`w-3 h-3 rounded-full border-2 ${vStyles.bg} ${isTop ? 'mb-1' : 'mt-1'}`}
          style={{ borderColor: strokeColor }}
        />
        <div
          className={`w-2 h-2 rounded-full border-2 ${vStyles.bg}`}
          style={{ borderColor: strokeColor }}
        />
      </div>
    );
  ...
  const avatarEl = avatarUrl ? (
    <div
      className={`${sStyles.avatarSize} shrink-0 border-2 border-black overflow-hidden shadow-[2px_2px_0px_#000] bg-black`}
    >
      <img
        src={avatarUrl}
  ...
  return (
    ...
    >
      {/* SFX Action Burst Badge (Floating on top corner) */}
      {sfx && (
      ...
      <div
        className={`relative ${vStyles.bg} ${vStyles.text} ${vStyles.border} ${
          vStyles.shadow
        } ${getShapeClasses()} transition-all duration-150 overflow-hidden`}
      >
        {/* SVG Contour & Border Decals */}
        {renderSvgBorderAccents()}

        {/* Scanline Effect for Vegapunk Terminal */}
        {isTerminal && (
        ...
        {/* Subtle Screentone Texture (Paper) */}
        ...
        {/* Header Strip with Vegapunk Satellite or Character Speaker Details */}
        {isTerminal ? (
            ...
            <div className="flex items-center space-x-2 min-w-0">
              ...
              <div className="min-w-0 truncate">
                <div className="flex items-center space-x-1.5">
                  ...
                  <span className="text-[9px] px-1 bg-[#00f2ff]/20 text-[#00f2ff] border border-[#00f2ff]/40">
                    SATELLITE
                  </span>
                </div>
            ...
            <div className="flex items-center space-x-1.5 shrink-0">
              ...
              {showCopyButton && (
                <button
                  type="button"
                  onClick={handleCopy}
                  className="p-1 hover:bg-[#00f2ff]/20 text-cyan-300 transition-colors cursor-pointer"
          ...
          (speaker || epithet || citation || showCopyButton || showAudioButton) && (
            <div
              className={`flex items-center justify-between border-b-2 ${
                vStyles.border
              } px-3.5 py-1.5 gap-2 rounded-t-[inherit] ${
                effectiveVariant === 'paper' ? 'bg-[#f0ece1]' : 'bg-black/30'
              } relative z-10`}
            >
              <div className="flex items-center space-x-2 min-w-0">
                ...
                <div className="min-w-0 truncate">
                  {speaker && (
                    <span className="font-manga text-xs sm:text-sm font-black tracking-wide uppercase block truncate leading-tight">
                      {speaker}
                    </span>
                  )}
                  {epithet && (
                    <span className="text-[10px] sm:text-[11px] font-heading font-bold opacity-75 block truncate leading-tight">
                      {epithet}
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center space-x-1.5 shrink-0 text-xs">
                {citation && (
                  <span
                    className={`text-[10px] font-mono px-1.5 py-0.5 border border-black/50 font-bold ${vStyles.badgeBg}`}
                  >
                    {citation}
                  </span>
                )}
